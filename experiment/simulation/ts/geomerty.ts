namespace Chemistry {
   ...
   export class Needle extends Geometry {
        needleLength: number = 150;
        needleWidth: number = 4;
        needleColor: string = "red";
        needleX: number = 0;
        needleY: number = 0;
        angleInRadians: number = 0;  // in radians
        canvas: HTMLCanvasElement;
        context : CanvasRenderingContext2D;

        constructor(x: number, y: number, l: number, w: number, angle: number, color: string, canvas: HTMLCanvasElement) {
            super();
