let curr_hall_current_index: number = 0;

let cv_meter_up_1: Chemistry.Circle;
let cv_meter_down_1: Chemistry.Circle;
let cv_meter_power_1: Chemistry.Circle;

let cv_power_on_1: boolean = true;

let can_use_cv_meter_1: boolean = false;
let can_mount_hall_probe_1: boolean = false;
let can_show_cv_reading_1s_1: boolean = false;
let a22_cv_down: boolean = false;
let a22_cv_up: boolean = false;
let a22_em_down: boolean = false;
let a22_em_up: boolean = false;

let cv_i_reading_1: Chemistry.Geo_Text;
let cv_v_reading_1: Chemistry.Geo_Text;
let i_reading_1: number;
let v_reading_1: number;

let seleted_em_index: number = 0;

let selected_data_array_1: number[][] = [];

let curr_data2_index_1: number = 0;

let can_edit_table: boolean = false;
let selected_mag_field: number = 0;
...
function load_buttons_2() {
	...
	cv_meter_power_1 = new Chemistry.Circle(
		new Chemistry.Point(1413, 200),
		15,
		canvas
	);
	cv_meter_up_1 = new Chemistry.Circle(
		new Chemistry.Point(1634, 175),
		15,
		canvas
	);
	cv_meter_up_1.color = 'transparent';
	cv_meter_down_1 = new Chemistry.Circle(
	...
	if (a22_em_up) {
		if (x <= 1205 && x >= 1175) {
			if (y <= -336 && y >= -367) {
				...
				let show_field: HTMLSpanElement = <HTMLSpanElement>(
					document.getElementById('show-field')
				);
...
function show_gm_display_2() {
	cv_i_reading_1 = new Chemistry.Geo_Text(
		'0.5',
		new Chemistry.Point(1455, 250),
		canvas
	);
	cv_v_reading_1 = new Chemistry.Geo_Text(
		'0.0',
...
function get_data_array_1() {
	for (let i = 0; i < data2.length; i++) {
		if (
			data3[i]['probe'] == selected_material &&
...
function dsp_reading_1() {
	if (can_show_cv_reading_1s_1) {
		...
		console.log(
			selected_data_array_1[curr_hall_current_index][1],
			seleted_em_index + 1
		);

		cv_v_reading_1.text =
			selected_data_array_1[curr_hall_current_index][
				seleted_em_index + 1
			].toFixed(1);
	}
}
