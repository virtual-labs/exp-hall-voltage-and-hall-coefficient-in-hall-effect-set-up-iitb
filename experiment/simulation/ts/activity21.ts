let curr_em_voltage_index: number = 0;

let cv_meter_up: Chemistry.Circle;
let cv_meter_down: Chemistry.Circle;
let cv_meter_power: Chemistry.Circle;

let cv_power_on: boolean = true;
let can_use_cv_meter: boolean = false;
let can_mount_hall_probe: boolean = false;
let can_show_cv_readings: boolean = false;

let a21_em_up: boolean = false;
let a21_em_down: boolean = false;
let a21_cv_up: boolean = false;
let a21_cv_down: boolean = false;

let cv_i_reading: Chemistry.Geo_Text;
let cv_v_reading: Chemistry.Geo_Text;
let i_reading: number;
let v_reading: number;

let selected_sim_current_index: number = 0;

let selected_data_array: number[][] = [];

let curr_data2_index: number = 0;

let can_edit_table_0: boolean = false;

let em_curr_show: number = 0;
...
function show_table_1() {
	...
	tb.innerHTML = `
    <p style='margin: 0; font-size: 1.4vw;'>Selected Hall Current = <span id='show-sel-current' style='color: blue; font-weight: 600;'>${
		selected_sim_current_index + 2
	} mA</span></p>
...
function load_buttons_1() {
	...
	cv_meter_power = new Chemistry.Circle(
		new Chemistry.Point(1413, 200),
		15,
		canvas
	);
	cv_meter_up = new Chemistry.Circle(
		new Chemistry.Point(1634, 175),
		15,
		canvas
	);
	cv_meter_up.color = 'transparent';
	cv_meter_down = new Chemistry.Circle(
		new Chemistry.Point(1551, 175),
		15,
		canvas
	);
		...
		if (x <= 1205 && x >= 1175) {
			if (y <= -336 && y >= -367) {
				...
				if (needle_angle < 150) {
					...
					curr_em_voltage_index = Math.round(
						(150 - needle_angle) / 2.42857
					);
				...
				let ele: HTMLSpanElement = <HTMLSpanElement>(
					document.getElementById('show-em-current')
				);
				ele.innerHTML =
	...
	if (cv_power_on && a21_cv_up) {
		if (x <= 1647 && x >= 1617) {
			if (y <= -408 && y >= -442) {
				...
				let show: HTMLSpanElement = <HTMLSpanElement>(
					document.getElementById('show-sel-current')
				);
...
function show_gm_display_1() {
	cv_i_reading = new Chemistry.Geo_Text(
		'2',
		new Chemistry.Point(1455, 250),
		canvas
	);
	cv_v_reading = new Chemistry.Geo_Text(
		'2.1',
		new Chemistry.Point(1616, 250),
		canvas
	);
...
function add_reading_1() {
	if (can_edit_table_0) {
		...
		alert(
			'For adding observations all power supplies must be on with non-zero values. Please check again!!'
		);
	}
...
function get_data_array() {
	for (let i = 0; i < data2.length; i++) {
		if (
			data2[i]['probe'] == selected_material &&
...
function dsp_reading() {
	if (can_show_cv_readings) {
		...
		console.log(
			selected_data_array[curr_em_voltage_index][1],
			selected_sim_current_index + 3
		);

		cv_v_reading.text =
			selected_data_array[curr_em_voltage_index][
				selected_sim_current_index + 3
			].toFixed(1);
	}
}
