...
class Observation_Table {
    constructor(heading_column, data) {
        this.template = ` 
    <div style=''>   
    <table class="table" style="position: absolute; height: 30vw; width: 32vw; top: 10vw; left: 5vw; font-size: 1.3vw;">
        <thead>
            <tr id="header-1">
            </tr>
        </thead>
    ...
    draw() {
        ...
        let row = '';
        for (let i = 0; i < this.heading_column.length; i++) {
            row += `<th style='padding: 0;' >${this.heading_column[i]}</th>`;
        }
        document.getElementById('header-1').innerHTML = row;
        document.getElementById('table-body').innerHTML = '';
        for (let i = 0; i < this.data.length; i++) {
            let col = `<tr>`;
            for (let j = 0; j < this.data[i].length; j++) {
                col += `<td style='padding: 0;' >${this.data[i][j]}</td>`;
            }
            col += `</tr>`;
            document.getElementById('table-body').innerHTML += col;
        }
